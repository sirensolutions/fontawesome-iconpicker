...
(function(a) {
    ...
    c.prototype = {
        ...
        _createIconpicker: function() {
            ...
            //If no custom icon exists, then the original icons are not replaced even when the replaceOriginalIcon is set to true
            if(this.options.replaceOriginalIcons && this.options.customIcons.length > 0){
              this.options.icons = this.options.customIcons;
            }
            if(!this.options.replaceOriginalIcons){
              //even if no icon list is passed, a concatenation with empty array makes no difference
              this.options.icons = this.options.icons.concat(this.options.customIcons);
            }
